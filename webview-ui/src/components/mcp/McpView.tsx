...
import { TabNavbarExample } from "../settings/TabNavbarExample"
...
interface McpViewProps {
	...
	onToolChange: (toolName: string, isAutoApprove: boolean) => void;
}
...
export const McpView: React.FC<McpViewProps> = ({
	...
	const { mcpServers: servers, mcpMarketplaceCatalog, autoApprovalSettings } = useExtensionState()
	const [activeTab, setActiveTab] = useState<'tools' | 'marketplace'>('tools');

	const handleTabChange = (tab: string) => {
		setActiveTab(tab as 'tools' | 'marketplace');
	}
		...
		<div
			...
			<div style={{ flex: 1, overflow: "auto" }}>
				...
				<div
					...
					<VSCodeButton
						appearance={activeTab === 'tools' ? 'primary' : 'secondary'}
					...
					{mcpMarketplaceEnabled && (
						<VSCodeButton
							appearance={activeTab === 'marketplace' ? 'primary' : 'secondary'}
						...
						<div className="mcp-tools-list">
							{tools.map((tool) => (
								<McpToolRow
									key={tool.name}
									toolName={tool.name}
									isAutoApprove={tool.isAutoApprove}
									onAutoApproveChange={(isAutoApprove) => onToolChange(tool.name, isAutoApprove)}
								/>
							))}
						</div>
					)}
