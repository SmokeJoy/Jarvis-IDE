...
-   Fix bug where Jarvis IDE would use plan_mode_response bug without response parameter
...
-   Add 'Add to Jarvis IDE' as an option when you right-click in a file or the terminal, making it easier to add context to your current task
-   Add 'Fix with Jarvis IDE' code action - when you see a lightbulb icon in your editor, you can now select 'Fix with Jarvis IDE' to send the code and associated errors for Jarvis IDE to fix. (Cursor users can also use the 'Quick Fix (CMD + .)' menu to see this option)
-   Add Account view to display billing and usage history for Jarvis IDE account users. You can now keep track of credits used and transaction history right in the extension!
-   Add 'Sort underling provider routing' setting to Jarvis IDE/OpenRouter allowing you to sort provider used by throughput, price, latency, or the default (combination of price and uptime)
-   Improve rich MCP display with dynamic image loading and support for GIFs
-   Add 'Documentation' menu item to easily access Jarvis IDE's docs
-   Add OpenRouter's new usage_details feature for more reliable cost reporting
-   Display total space Jarvis IDE takes on disk next to 'Delete all Tasks' button in History view
-   Fix 'Context Window Exceeded' error for OpenRouter/Jarvis IDE Accounts (additional support coming soon)
...
-   Jarvis IDE now displays selectable options when asking questions or presenting a plan, saving you from having to type out responses!
-   Add support for a `.jarvisrules/` directory to load multiple files at once (thanks @ryo-ma!)
-   Prevent Jarvis IDE from reading extremely large files into context that would overload context window
...
-   Add Jarvis IDE API as a provider option, allowing new users to sign up and get started with Jarvis IDE for free
...
-   Improve OpenRouter/Jarvis IDE error reporting
...
-   Add .jarvisignore to block Jarvis IDE from accessing specified file patterns
...
-   Add Plan/Act mode toggle to let you plan tasks with Jarvis IDE before letting him get to work
...
-   Add ability to change viewport size and headless mode when Jarvis-IDE asks to launch the browser
...
-   Fix bug where Jarvis-IDE couldn't read "@/" import path aliases from tool results
...
-   Added checkpoints: Snapshots of workspace are automatically created whenever Jarvis IDE uses a tool
...
-   Jarvis IDE now uses a search & replace diff based approach when editing large files to prevent code deletion issues.
...
-   Adds ability to enable system notifications for when Jarvis IDE needs approval or completes a task.
-   Adds support for a root-level `.jarvisrules` file that can be used to specify custom instructions for the project.

## [2.2.0]

-   Add support for Model Context Protocol (MCP), enabling Jarvis IDE to use custom tools like web-search tool or GitHub tool
-   Add MCP server management tab accessible via the server icon in the menu bar
-   Add ability for Jarvis IDE to dynamically create new MCP servers based on user requests (e.g., "add a tool that gets the latest npm docs")
...
-   Add stricter prompt to prevent Jarvis IDE from editing files during a browser session without first closing the browser

## [2.1.0]

-   Jarvis IDE now uses Anthropic's new "Computer Use" feature to launch a browser, click, type, and scroll. This gives it more autonomy in runtime debugging, end-to-end testing, and even general web use. Try asking "Look up the weather in Colorado" to see it in action! (Available with Claude 3.5 Sonnet v2)
...
-   Fix bug where modifying Jarvis IDE's edits would lead it to try to re-apply the edits
...
-   Update system prompt to try to prevent Jarvis IDE from lazy coding (`// rest of code here...`)
...
-   Fixed bug where Jarvis IDE's edits would stream into the active tab when switching tabs during a write_to_file
...
-   New name! Meet Jarvis IDE, an AI assistant that can use your CLI and Editor
-   Responses are now streamed with a yellow text decoration animation to keep track of Jarvis IDE's progress as it edits files
-   New Cancel button to give Jarvis IDE feedback if it goes off in the wrong direction, giving you more control over tasks
...
-   Jarvis IDE can now use a browser! This update adds a new `inspect_site` tool that captures screenshots and console logs from websites (including localhost), making it easier for Jarvis IDE to troubleshoot issues on its own.
-   Improved automatic linter/compiler debugging by only sending Jarvis IDE new errors that result from its edits, rather than reporting all workspace problems.
...
-   Commands now run directly in your terminal thanks to VSCode 1.93's new shell integration updates! Plus a new 'Proceed While Running' button to let Jarvis IDE continue working while commands run, sending it new output along the way (i.e. letting it react to server errors as it edits files)

## [1.5.27]

-   Jarvis IDE's changes now appear in your file's Timeline, allowing you to easily view a diff of each edit. This is especially helpful if you want to revert to a previous version. No need for git—everything is tracked by VSCode's local history!
-   Updated system prompt to keep Jarvis IDE from re-reading files unnecessarily
...
-   New terminal emulator! When Jarvis IDE runs commands, you can now type directly in the terminal (+ support for Python environments)
-   Adds search to Task History

## [1.5.6]

-   You can now edit Jarvis IDE's changes before accepting! When it edits or creates a file, you can modify its changes directly in the right side of the diff view (+ hover over the 'Revert Block' arrow button in the center to undo `// rest of code here` shenanigans)
...
-   Adds new `search_files` tool that lets Jarvis IDE perform regex searches in your project, making it easy for it to refactor code, address TODOs and FIXMEs, remove dead code, and more!

## [1.4.0]

-   Adds "Always allow read-only operations" setting to let Jarvis IDE read files and view directories without needing approval (off by default)
...
-   Paste images in chat to use Jarvis IDE's vision capabilities and turn mockups into fully functional applications or fix bugs with screenshots
...
-   Jarvis IDE now has context about vscode's visible editors and opened tabs

## [1.0.4]

-   Open in the editor (using menu bar or `Jarvis IDE Dev: Open In New Tab` in command palette) to see how Jarvis IDE updates your workspace more clearly
-   New `analyze_project` tool to help Jarvis IDE get a comprehensive overview of your project's source code definitions and file structure
