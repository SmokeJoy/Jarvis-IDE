  ...
  describe('extractModulesFromImports', () => {
    test('dovrebbe estrarre correttamente i moduli da vari tipi di dichiarazioni di importazione', () => {
      ...
      expect(result).toContainEqual(
        expect.objectContaining({ name: 'react', isDynamicImport: false })
      );
      expect(result).toContainEqual(
        expect.objectContaining({ name: 'path', isDynamicImport: false })
      );
      expect(result).toContainEqual(
        expect.objectContaining({ name: '../types/user', isDynamicImport: false })
      );
      expect(result).toContainEqual(
        expect.objectContaining({ name: './dynamicModule', isDynamicImport: true })
      );
      expect(result).toContainEqual(
        expect.objectContaining({ name: 'fs', isDynamicImport: false })
      );
      expect(result).toContainEqual(
        expect.objectContaining({ name: './Button', isDynamicImport: false })
      );
      expect(result).toContainEqual(
        expect.objectContaining({ name: './utils', isDynamicImport: false })
      );
    });
  });
  ...
  describe('fixImportsInFile', () => {
    test('dovrebbe aggiungere .js alle importazioni relative in file TS/TSX', async () => {
      ...
      (fs.existsSync as jest.Mock).mockImplementation((filePath: string) => {
        ...
        if (
          filePath.includes('components/Component.ts') ||
      ...
      expect(fs.promises.writeFile).toHaveBeenCalledWith(
        'src/someFile.ts',
        expect.stringContaining("import type { Component } from '../components/Component';"),
        'utf8'
      );
      ...
      expect(fs.promises.writeFile).toHaveBeenCalledWith(
        'src/someFile.ts',
        expect.stringContaining("import * as constants from '../constants';"),
        'utf8'
      );
    ...
    test('dovrebbe gestire correttamente i percorsi index', async () => {
      ...
      (fs.promises.readFile as jest.Mock).mockResolvedValue(fileContent);
      (fs.existsSync as jest.Mock).mockImplementation((filePath: string) => {
        if (filePath.includes('components/index.ts') || filePath.includes('utils/index.ts')) {
          return true;
        }
        return false;
      });
      ...
      expect(fs.promises.writeFile).toHaveBeenCalledWith(
        'src/someFile.ts',
        expect.stringContaining("import { Button } from '../components/index';"),
        'utf8'
      );

      expect(fs.promises.writeFile).toHaveBeenCalledWith(
        'src/someFile.ts',
        expect.stringContaining("import { utils } from './utils/index';"),
        'utf8'
      );
    });
    ...
    test('dovrebbe misurare correttamente il tempo di esecuzione', async () => {
      ...
      const fileContent = `import type { Component } from '../components/Component';`;
      (fs.promises.readFile as jest.Mock).mockResolvedValue(fileContent);
      (fs.existsSync as jest.Mock).mockReturnValue(true);
      (fs.promises.stat as jest.Mock).mockResolvedValue({ isDirectory: () => false });
