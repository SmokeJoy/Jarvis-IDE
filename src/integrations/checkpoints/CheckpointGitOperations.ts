 ...
 * GitOperations Class
 *
 * Handles git-specific operations for Jarvis IDE's Checkpoints system.
 *
  ...
  public async initShadowGit(gitPath: string, cwd: string, taskId: string): Promise<string> {
    ...
    if (await fileExistsAtPath(gitPath)) {
      ...
      if (worktree.value !== cwd) {
        throw new Error(
          'Checkpoints can only be used in the original workspace: ' + worktree.value
        );
      }
    ...
    await git.addConfig('user.name', 'Jarvis IDE Checkpoint');
    await git.addConfig('user.email', 'checkpoint@jarvis-ide.dev');
