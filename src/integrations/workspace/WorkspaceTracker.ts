...
class WorkspaceTracker {
	private providerRef: WeakRef<JarvisProvider>
	private disposables: vscode.Disposable[] = []
	private filePaths: Set<string> = new Set()

	constructor(provider: JarvisProvider) {
		this.providerRef = new WeakRef(provider)
		this.registerListeners()
	}

	async populateFilePaths() {
		// should not auto get filepaths for desktop since it would immediately show permission popup before jarvis-ide ever creates a file
		if (!cwd) {
			return
		}
	...
	private registerListeners() {
		 ...
		 */
		// In other words, we don't have to worry about the root workspace folder ([0]) changing since the extension will be restarted and our cwd will be updated to reflect the new workspace folder. (We don't care about non root workspace folders, since jarvis-ide will only be working within the root folder cwd)
