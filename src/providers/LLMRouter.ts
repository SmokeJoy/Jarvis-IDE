...
export function getProvider(name: string, options: ProviderOptions = {}): BaseLLMProvider {
  ...
  if (!isProviderSupported(providerName)) {
    throw new Error(`Provider LLM non supportato: ${name}. Provider disponibili: ${SUPPORTED_PROVIDERS.join(', ')}`);
  }
  ...
  switch (providerName) {
    ...
    case PROVIDER_NAMES.GGUF:
      if (!options.ggufConfig) {
        throw new Error(`${PROVIDER_NAMES.GGUF.toUpperCase()}Provider richiede ggufConfig con binaryPath e modelsPath`);
      }
