...
export function calculateApiCostInternal(
  ...
  const inputCost =
    ((modelInfo.inputPrice || (modelInfo.pricing?.input ?? 0)) / 1_000_000) * inputTokens;
  const outputCost =
    ((modelInfo.outputPrice || (modelInfo.pricing?.output ?? 0)) / 1_000_000) * outputTokens;
...
export function calculateApiCostAnthropic(
  ...
  return calculateApiCostInternal(modelInfo, {
    inputTokens,
    outputTokens,
    cacheCreationInputTokens: cacheCreationInputTokensNum,
    cacheReadInputTokens: cacheReadInputTokensNum,
  });
}
