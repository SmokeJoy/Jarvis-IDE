...
export function toMarkdown(session: ExportableSession): string {
  try {
    ...
    if (session.messages && session.messages.length > 0) {
      ...
      messagesSection += session.messages.map((message) => formatMessage(message)).join('---\n\n');
    }

    // Componi il documento completo
    const markdownContent = [header, settingsSection, contextFilesSection, messagesSection]
      .filter(Boolean)
      .join('');
