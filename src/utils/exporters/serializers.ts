...
export function fromYAML(yamlString: string): any {
  try {
    ...
    throw new Error(`Errore durante l'analisi YAML: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
export function fromJSON(jsonString: string): any {
  try {
    ...
    throw new Error(`Errore durante l'analisi JSON: ${error instanceof Error ? error.message : String(error)}`);
  }
} 