...
import { fromJSON, fromYAML } from './serializers.js';
import { ExportFormat, ExportableSession, ExportError } from './types.js';
...
export function validateExportableSession(session: unknown): session is ExportableSession {
  ...
  if (typedSession.settings !== undefined && 
...
export function importFromString(
  ...
  try {
    ...
    Logger.error(`Errore nell'importazione da ${format}: ${error instanceof Error ? error.message : String(error)}`);
    throw error instanceof ExportError
      ? error
      : new ExportError(`Errore nell'importazione da ${format}: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
function importFromMarkdown(content: string): ExportableSession {
  ...
  if (filesMatch) {
    ...
    session.contextFiles = filesLines.map(line => {
...
function importFromHTML(content: string): ExportableSession {
  ...
  const messagePattern = /<div class="message ([^"]+)"[^>]*>(?:[\s\S]*?)<div class="role">[^<]*<\/div>(?:[\s\S]*?)<div class="content"><p>([\s\S]*?)<\/p><\/div>(?:[\s\S]*?)<\/div>/g;
  let match;
