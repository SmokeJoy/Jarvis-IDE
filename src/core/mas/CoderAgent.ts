...
export class CoderAgent implements ICoderAgent {
  ...
  handleMessage(message: AgentMessage): void {
    ...
    if (message.from !== 'supervisor-agent') {
      this.logger.warn(`Messaggio ricevuto da ${message.from} ignorato: CoderAgent accetta messaggi solo dal SupervisorAgent`);
      return;
    }
    ...
    switch (message.type) {
      case 'instruction':
        ...
        if (typeof message.payload === 'string') {
          this.executeInstruction(message.payload)
            .catch(error => this.logger.error(`Errore nell'esecuzione: ${error}`));
        } else if (typeof message.payload === 'object') {
          this.currentInstruction = message.payload as CoderInstruction;
          this.executeInstruction(this.currentInstruction.objective)
            .catch(error => this.logger.error(`Errore nell'esecuzione: ${error}`));
        }
        break;
