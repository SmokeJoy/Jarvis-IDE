...
import type { JarvisIdeApiReqInfo, JarvisIdeMessage } from "../../shared/ExtensionMessage.js.js"
import type { ApiHandler } from "../../api.js.js"
import type { OpenAiHandler } from "../../api/providers/openai.js.js"

export class ContextManager {
	getNewContextMessagesAndMetadata(
		apiConversationHistory: Anthropic.Messages.MessageParam[],
		jarvisIdeMessages: JarvisIdeMessage[],
		api: ApiHandler,
		...
		if (previousApiReqIndex >= 0) {
			const previousRequest = jarvisIdeMessages[previousApiReqIndex]
			if (previousRequest && previousRequest.text) {
				const { tokensIn, tokensOut, cacheWrites, cacheReads }: JarvisIdeApiReqInfo = JSON.parse(previousRequest.text)
	...
	public getNextTruncationRange(
		...
		// NOTE: anthropic format messages are always user-assistant-user-assistant, while openai format messages can have multiple user messages in a row (we use anthropic format throughout jarvis-ide)
