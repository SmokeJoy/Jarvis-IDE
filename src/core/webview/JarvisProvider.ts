...
import { WebviewMessage, OutgoingWebviewMessage, convertToWebviewMessage } from "../../shared/protocols/index.js"
...
export class JarvisProvider implements IJarvisProvider {
	 ...
	 */
	private initMasSystem(): SupervisorAgent {
		if (!this.masSystem) {
	...
	private setupMasEventListeners(): void {
		...
		this.masSystem.on('instruction-queued', (data: MASInstructionData) => {
		...
		this.masSystem.on('instruction-completed', (data: MASInstructionData) => {
			this.postMessageToWebview({
		...
		this.masSystem.on('instruction-failed', (data: MASInstructionData) => {
			this.postMessageToWebview({
