...
import { ExtensionMessageType, ExtensionMessage, isExtensionMessage } from '@/shared/types/extensionMessageUnion';
import { mockModelInfo, mockExtensionSettings, mockChatMessages, mockMessageCreator } from './testUtils';
...
vi.mock('vscode', () => ({
  ...
  ExtensionContext: vi.fn().mockImplementation((): MockedContext => ({
    subscriptions: [],
    workspaceState: {
      get: vi.fn(),
      update: vi.fn()
...
describe('JarvisProvider', () => {
  ...
  describe('Gestione messaggi con type guard', () => {
    test('dovrebbe usare isExtensionMessage per validare i messaggi', async () => {
      ...
      const message = mockMessageCreator.createLogMessage('info', 'Test log message', { test: true });
  ...
  describe('Dispatcher type-safe', () => {
    ...
    test('dovrebbe dispatchare correttamente gli aggiornamenti del modello', async () => {
      ...
      const message = mockMessageCreator.createModelUpdate(mockModelInfo.id, mockModelInfo, 'ready');
      await provider.handleWebviewMessage(message);
    ...
    test('dovrebbe dispatchare correttamente gli aggiornamenti della chat', async () => {
      ...
      const message = mockMessageCreator.createChatUpdate('test-thread', mockChatMessages, 'active');
      await provider.handleWebviewMessage(message);
  ...
  describe('postMessageToWebview', () => {
    ...
    test('dovrebbe loggare un warning se la webview non è disponibile', () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith('WebView non disponibile, impossibile inviare il messaggio');
