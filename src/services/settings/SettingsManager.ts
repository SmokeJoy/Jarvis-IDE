...
export class SettingsManager {
    ...
    public async loadSettings(): Promise<JarvisSettings> {
        try {
            ...
            if (this.settings.contextPrompt && typeof this.settings.contextPrompt === 'string' 
                && this.settings.contextPrompt.length > 0) {
                // Verifica se esiste già un profilo di default
                const defaultProfileIndex = this.settings.promptProfiles.findIndex(p => p.id === 'default');
                
                if (defaultProfileIndex >= 0) {
                    // Aggiorna il profilo esistente con la stringa legacy come prompt di sistema
                    this.settings.promptProfiles[defaultProfileIndex].contextPrompt.system = this.settings.contextPrompt;
     ...
     */
    public async updatePromptProfile(id: string, updates: Partial<PromptProfile>): Promise<PromptProfile> {
        const profiles = this.getPromptProfiles();
