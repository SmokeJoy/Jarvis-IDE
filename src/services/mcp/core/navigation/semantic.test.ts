...
describe('findSemanticPath', () => {
  ...
  it('dovrebbe gestire correttamente i contesti inesistenti', async () => {
    await expect(findSemanticPath('ctx-999', 'ctx-1')).rejects.toThrow(
      'Contesto di partenza con ID ctx-999 non trovato'
    );

    await expect(findSemanticPath('ctx-1', 'ctx-999')).rejects.toThrow(
      'Contesto di destinazione con ID ctx-999 non trovato'
    );
  });
  ...
  it('dovrebbe rispettare i filtri su strength e confidence', async () => {
    ...
    expect(result.success).toBe(true);
    expect(
      result.path?.edges.every((edge) => edge.strength! >= 0.7 && edge.confidence! >= 0.8)
    ).toBe(true);
  });
