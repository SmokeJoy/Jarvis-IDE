...
import { WebviewMessage } from '../../shared/protocols';
...
export async function testMcpDispatcher() {
  ...
  if (tool === 'read_file') {
    ...
    const useRegex = await vscode.window.showQuickPick(['No', 'Sì'], {
      placeHolder: 'Usare regex?',
      canPickMany: false,
    });

    // Chiedi il numero massimo di risultati
    const maxResults = await vscode.window.showQuickPick(['3', '5', '10', '20'], {
      placeHolder: 'Numero massimo di risultati',
      canPickMany: false,
    });
    ...
    vscode.window.showInformationMessage(
      `Invio richiesta per cercare: ${query} (regex: ${useRegex === 'Sì' ? 'sì' : 'no'})`
    );
  } else if (tool === 'memory.query') {
    // Chiedi l'ambito
    const scope = await vscode.window.showQuickPick(['chat', 'project', 'agent', 'all'], {
      placeHolder: "Seleziona l'ambito della memoria",
      canPickMany: false,
    });
    ...
    // Chiedi il limite
    const limit = await vscode.window.showQuickPick(['3', '5', '10', '20'], {
      placeHolder: 'Numero massimo di risultati',
      canPickMany: false,
    });
    ...
    vscode.window.showInformationMessage(
      `Invio richiesta per memoria: ${scope}${filterInfo} (max: ${limit || '5'})`
    );
    ...
    vscode.window.showInformationMessage(
      `Invio richiesta per generare codice ${language}${contextInfo}`
    );
    ...
    const overwrite = await vscode.window.showQuickPick(['No', 'Sì'], {
      placeHolder: 'Sovrascrivere se esiste?',
      canPickMany: false,
    });

    // Chiedi se usare preview
    const previewOnly = await vscode.window.showQuickPick(['No', 'Sì'], {
      placeHolder: 'Solo anteprima (senza scrivere)?',
      canPickMany: false,
    });
    ...
    const modeInfo =
      previewOnly === 'Sì' ? 'anteprima' : overwrite === 'Sì' ? 'sovrascrittura' : 'scrittura';
    ...
    const explanation = await vscode.window.showQuickPick(['Sì', 'No'], {
      placeHolder: 'Includere spiegazione delle modifiche?',
      canPickMany: false,
    });
    ...
    vscode.window.showInformationMessage(
      `Invio richiesta per refactoring ${language}: ${objective}`
    );
    ...
    const maxSourceFiles = await vscode.window.showQuickPick(['3', '5', '10', '15'], {
      placeHolder: 'Numero massimo di file sorgente',
      canPickMany: false,
    });

    // Chiedi se includere snippets di codice
    const includeCode = await vscode.window.showQuickPick(['Sì', 'No'], {
      placeHolder: 'Includere snippets di codice?',
      canPickMany: false,
    });
    ...
    const filterInfo = filter ? ` con filtro "${filter}"` : '';
    vscode.window.showInformationMessage(
      `Invio richiesta per documentazione: ${question}${filterInfo}`
    );
    ...
    const shouldFix = await vscode.window.showQuickPick(['No', 'Sì'], {
      placeHolder: 'Applicare le correzioni automatiche?',
      canPickMany: false,
    });
    ...
    const langInfo =
      language === 'Auto-rilevamento' ? ' (linguaggio auto-rilevato)' : ` (${language})`;
    const fixInfo = shouldFix === 'Sì' ? ' con correzioni automatiche' : '';
    vscode.window.showInformationMessage(
      `Invio richiesta per analisi linting: ${path}${langInfo}${fixInfo}`
    );
    ...
    const write = await vscode.window.showQuickPick(['No', 'Sì'], {
      placeHolder: 'Scrivere il file formattato?',
      canPickMany: false,
    });
    ...
    vscode.window.showInformationMessage(
      `Invio richiesta per formattazione: ${path}${langInfo}${writeInfo}`
    );
    ...
    // Chiedi il framework
    const framework = await vscode.window.showQuickPick(['jest', 'pytest', 'mocha', 'vitest'], {
      placeHolder: 'Framework di test da utilizzare',
      canPickMany: false,
    });
    ...
    const coverage = await vscode.window.showQuickPick(['No', 'Sì'], {
      placeHolder: 'Generare report di copertura?',
      canPickMany: false,
    });
    ...
    // Chiedi il formato
    const format = await vscode.window.showQuickPick(['json', 'dot', 'tree'], {
      placeHolder: 'Formato di output',
      canPickMany: false,
    });
    ...
    // Chiedi lo scope
    const scope = await vscode.window.showQuickPick(['chat', 'project', 'agent'], {
      placeHolder: 'Seleziona lo scope della memoria',
      canPickMany: false,
    });
    ...
    vscode.window.showInformationMessage(
      `Invio richiesta per iniettare contesto in scope '${scope}'`
    );
    ...
    const wantTags = await vscode.window.showQuickPick(['No', 'Sì'], {
      placeHolder: 'Vuoi filtrare per tag?',
      canPickMany: false,
    });
    ...
    if (clearMode === 'Cancella per ID') {
      ...
      const wantScope = await vscode.window.showQuickPick(['No', 'Sì'], {
        placeHolder: 'Limitare la ricerca a uno scope specifico?',
        canPickMany: false,
      });

      if (wantScope === 'Sì') {
        const scope = await vscode.window.showQuickPick(['chat', 'project', 'agent'], {
          placeHolder: 'Seleziona lo scope',
          canPickMany: false,
        });
      ...
      // Chiedi lo scope
      const scope = await vscode.window.showQuickPick(['chat', 'project', 'agent'], {
        placeHolder: 'Seleziona lo scope da cancellare completamente',
        canPickMany: false,
      });
      ...
      // Chiedi conferma
      const confirmation = await vscode.window.showQuickPick(['No', 'Sì'], {
        placeHolder: 'Sei sicuro di voler cancellare TUTTI i contesti in TUTTI gli scope?',
        canPickMany: false,
      });
    ...
    const tagsInput = await vscode.window.showInputBox({
      placeHolder: 'Tag (separati da virgola)',
      prompt:
        'Inserisci i tag da associare al contesto (es. architettura,performance,best-practice)',
    });
    ...
    vscode.window.showInformationMessage(
      `Invio richiesta per ${replace === 'Sì, sostituisci i tag esistenti' ? 'sostituire' : 'aggiungere'} tag al contesto con ID '${id}'`
    );
    ...
    const searchTags = searchTagsInput
      .split(',')
      .map((tag) => tag.trim())
      .filter((tag) => tag.length > 0);
    ...
    if (searchScope && searchScope !== 'Tutti gli scope')
      searchMessage += ` in scope '${searchScope}'`;
    ...
    const depth = await vscode.window.showQuickPick(['1', '2', '3', '4', '5'], {
      placeHolder: 'Profondità di scansione',
      canPickMany: false,
    });
    ...
    const includeFiles = await vscode.window.showQuickPick(['Sì', 'No'], {
      placeHolder: 'Includere analisi dei file chiave?',
      canPickMany: false,
    });
    ...
    vscode.window.showInformationMessage(
      `Invio richiesta per riepilogo progetto (profondità: ${depth})${filesInfo}`
    );
  } else {
