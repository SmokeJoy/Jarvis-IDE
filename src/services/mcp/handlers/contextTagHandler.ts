 ...
 */
function findContextById(id: string, specificScope?: string): { item: any, scope: string, index: number } | null {
  if (specificScope) {
 ...
 */
function addTagsToContext(id: string, tags: string[], replace: boolean = false): { 
  success: boolean, 
  context?: any, 
  scope?: string,
  addedTags?: string[], 
 ...
 */
export function getContextsByTags(tags: string[], specificScope?: string): any[] {
 ...
 */
export const contextTagHandler: McpToolHandler = async (args): Promise<McpToolResult> => {
  // Estrai i parametri
  const id = args?.id;
