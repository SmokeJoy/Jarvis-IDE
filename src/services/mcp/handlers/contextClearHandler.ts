 ...
 */
function removeItemById(id: string, scope?: string): boolean {
  if (scope) {
 ...
 */
function clearScope(scope: string): number {
  const items = getFromMemory(scope);
 ...
 */
function clearAllScopes(): Record<string, number> {
  const allMemory = getAllMemory();
 ...
 */
export const contextClearHandler: McpToolHandler = async (args): Promise<McpToolResult> => {
  // Estrai i parametri
  ...
  if (!id && !all) {
    return {
      ...
      output: null,
      error:
        "È necessario specificare almeno uno tra 'id' (per rimuovere un elemento specifico) e 'all' (per rimuovere tutti gli elementi)",
    };
  }
