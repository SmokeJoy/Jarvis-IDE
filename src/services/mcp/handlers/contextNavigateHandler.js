 ...
 */
async function findWeightedPath(startId, targetId, links, contexts, strategy, includeContent = true, includeMetadata = true) {
    // Costruisci il grafo
    ...
    while (queue.length > 0) {
        ...
        for (const edge of graph[current]) {
            ...
            if (strategy?.minStrength && (edge.strength || 0) < strategy.minStrength)
                continue;
    ...
    const edges = path.slice(1).map((targetId, i) => {
        ...
        if (!link)
            throw new Error(`Relazione non trovata tra ${sourceId} e ${targetId}`);
        return {
            sourceId,
            targetId,
...
async function navigateShortest(options, contexts, links) {
    if (!options.targetId) {
        return {
            success: false,
 ...
 */
async function findSemanticPath(startId, targetId, strategy, includeContent = true, includeMetadata = true) {
    // Cache per i contesti
    const contextCache = new Map();
    // Funzione helper per ottenere contesti con caching
    async function getCachedContext(id) {
    ...
    const calculateSemanticScore = (link, context) => {
        let score = 1; // Base
        // Punteggio per tag
    ...
    for (const link of links) {
        ...
        // Applica filtri
        if (strategy.minStrength && (link.strength || 0) < strategy.minStrength)
            continue;
    ...
    for (let i = 0; i < path.length; i++) {
        ...
        if (i < path.length - 1) {
            const link = links.find(l => (l.sourceId === path[i] && l.targetId === path[i + 1]) ||
 ...
 */
async function findExploratoryPath(startId, strategy, includeContent = true, includeMetadata = true, format = "graph") {
    // Cache per i contesti
    const contextCache = new Map();
    // Funzione helper per ottenere contesti con caching
    async function getCachedContext(id) {
    ...
    while (queue.length > 0) {
        ...
        const scoredLinks = await Promise.all(outgoingLinks.map(async (link) => {
    ...
    if (format === "tree") {
        ...
        const treeEdges = edges.filter(edge => parentMap.get(edge.targetId) === edge.sourceId ||
...
export async function contextNavigateHandler(startId, targetId, mode, options = {}, includeContent = false, includeMetadata = false, format = 'graph') {
    switch (mode) {
        case 'shortest':
        case 'weighted':
        case 'semantic':
...
//# sourceMappingURL=contextNavigateHandler.js.map