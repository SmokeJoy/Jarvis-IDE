 ...
 */
async function scanDirectory(dirPath: string, depth: number, currentDepth: number = 0): Promise<string> {
    ...
    try {
        ...
        if (jsdocMatch) {
            ...
            // Per i README, prova a estrarre i primi paragrafi
                ...
                description = firstParagraphs.length > 0 ? firstParagraphs : 'Contenuto README senza paragrafi distinguibili';
            }
            // Per file di codice, cerca commenti in cima o descrizione di classe/funzione
                ...
                if (lineComments) {
                    ...
                    const classOrFunctionMatch = content.match(/(?:export\s+)?(?:class|function|const|interface)\s+(\w+)/);
...
async function analyzeKeyFiles(workspaceRoot: string): Promise<string> {
    try {
        ...
        const rootFilesToAnalyze: string[] = [
            'package.json',
            'tsconfig.json',
            'README.md'
        ];
        ...
        if (srcExists) {
            ...
            const mainFiles = srcFiles.filter(f => f === 'index.ts' ||
                f === 'main.ts' ||
                f === 'app.ts' ||
                f === 'extension.ts');
