 ...
 */
function addToMemory(scope, text) {
    const newItem = {
        id: generateMemoryId(),
        scope,
        text,
 ...
 */
export function getFromMemory(scope) {
 ...
 */
async function persistMemoryToDisk() {
    try {
 ...
 */
export async function loadMemoryFromDisk() {
    try {
        // Ottieni il percorso workspace
        let workspacePath = process.cwd();
        if (vscodeMod.workspace.workspaceFolders && vscodeMod.workspace.workspaceFolders.length > 0) {
 ...
 */
export const contextInjectHandler = async (args) => {
    // Estrai i parametri
...
//# sourceMappingURL=contextInjectHandler.js.map