...
import { getFromMemory, getAllMemory } from "./contextInjectHandler";
import { getContextsByTags } from "./contextTagHandler";
/**
 * Crea una stringa di anteprima del testo (primi 50 caratteri)
 */
function createTextPreview(text, maxLength = 50) {
 ...
 */
export const contextListHandler = async (args) => {
    // Estrai i parametri
    ...
    try {
        ...
        if (filterText) {
            memoryItems = memoryItems.filter(item => item.text.toLowerCase().includes(filterText.toLowerCase()));
        }
        // Filtra per timestamp se specificato
        if (sinceTimestamp) {
            memoryItems = memoryItems.filter(item => item.timestamp >= sinceTimestamp);
        }
...
//# sourceMappingURL=contextListHandler.js.map