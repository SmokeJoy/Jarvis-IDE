...
describe('directoryScanner', () => {
  ...
  it('dovrebbe scansionare una directory con successo', async () => {
    ...
    (fs.readdir as jest.Mock).mockImplementation((dirPath, options) => {
      if (dirPath === '/test/directory') {
        ...
        return Promise.resolve([{ name: 'subfile.txt', isDirectory: () => false }]);
      }
    ...
    expect(
      result.data?.find((item: DirectoryScanResult) => item.path === '/test/directory/subdir')?.type
    ).toBe('directory');
    expect(
      result.data?.find((item: DirectoryScanResult) => item.path === '/test/directory/file1.txt')
        ?.type
    ).toBe('file');
  ...
  it('dovrebbe rispettare maxDepth', async () => {
    ...
    const subdir = result.data?.find(
      (item: DirectoryScanResult) => item.path === '/test/directory/subdir'
    );
  ...
  it('dovrebbe escludere directory basate sul pattern', async () => {
    ...
    (fs.readdir as jest.Mock).mockImplementation((dirPath, options) => {
      if (dirPath === '/test/directory') {
        ...
        return Promise.resolve([{ name: 'app.js', isDirectory: () => false }]);
      }
      return Promise.resolve([]);
    });
  ...
  it('dovrebbe utilizzare i valori di default quando non specificati', async () => {
    ...
    expect(result.data).toHaveLength(2);
    expect(result.data?.map((item: DirectoryScanResult) => item.path)).toContain(
