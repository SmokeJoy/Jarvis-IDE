...
import { JarvisIdeIgnoreController } from '../../core/ignore/JarvisIdeIgnoreController';
...
export async function parseSourceCodeForDefinitionsTopLevel(
  ...
  const allowedFilesToParse = jarvisIdeIgnoreController
    ? jarvisIdeIgnoreController.filterPaths(filesToParse)
    : filesToParse;

  for (const filePath of allowedFilesToParse) {
    const definitions = await parseFile(filePath, languageParsers, jarvisIdeIgnoreController);
    if (definitions) {
...
async function parseFile(
  ...
  if (jarvisIdeIgnoreController && !jarvisIdeIgnoreController.validateAccess(filePath)) {
    return null;
  }
  ...
  try {
    ...
    captures.sort(
      (
        a: { node: { startPosition: { row: number } } },
        b: { node: { startPosition: { row: number } } }
    ...
    captures.forEach(
      (capture: {
        node: { startPosition: { row: number }; endPosition: { row: number } };
        name: string;
      }) => {
