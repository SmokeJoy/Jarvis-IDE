...
export class BenchmarkStorage {
  ...
  async importFromJSON(
    filePath: string,
    description: string = 'Importato da file'
  ): Promise<string> {
    try {
  ...
  getProviderStats(daysBack: number = 30): any {
    ...
    for (const result of allResults) {
      ...
      if (type === 'call') {
        stats.callTests++;
        if (!error && duration > 0) {
          stats.avgCallDuration = updateAverage(stats.avgCallDuration, duration, stats.callTests);
        }
        if (!error && responseLength > 0) {
          stats.avgCallLength = updateAverage(stats.avgCallLength, responseLength, stats.callTests);
        }
      } else if (type === 'stream') {
        stats.streamTests++;
        if (!error && duration > 0) {
  ...
  private normalizeResult(
    data: any,
    inferredType?: 'call' | 'stream'
  ): Omit<BenchmarkResult, 'id'> {
    return {
