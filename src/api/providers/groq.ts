...
export class GroqHandler extends BaseStreamHandler implements ApiHandler {
	...
	constructor(options: ApiHandlerOptions) {
		...
		logger.debug(`[GroqHandler] Inizializzato con API key: ${this.options.groqApiKey ? '***' : 'non fornita'}`)
	}
	...
	protected async fetchAPIResponse(systemPrompt: string, messages: Anthropic.Messages.MessageParam[]): Promise<Stream<ChatCompletionChunk>> {
		...
		try {
			...
			logger.error(`[GroqHandler] Errore durante la chiamata API Groq: ${error instanceof Error ? error.message : String(error)}`)
			throw error
		}
	}
	...
	async *createMessage(systemPrompt: string, messages: Anthropic.Messages.MessageParam[]): ApiStream {
		...
		const apiStream = this.transformToApiStream(rawStream, chunk => {
			...
			if (chunk && 'usage' in chunk && chunk.usage) {
				...
				logger.debug(`[GroqHandler] Informazioni di utilizzo: input=${inputTokens}, output=${outputTokens}, costo=${totalCost}`)
