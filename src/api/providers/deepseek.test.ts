...
describe('DeepSeekHandler', () => {
  ...
  it('✅ should yield reasoning chunks (deepseek-reasoner)', async () => {
    ...
    handler.setMockStream(
      createMockStream([{ choices: [{ delta: { reasoning_content: 'Spiegazione logica' } }] }])
    );
  ...
  it('✅ should retry on transient errors', async function () {
    ...
    handler.setMockStream(
      createMockStream([{ choices: [{ delta: { content: 'Risposta dopo retry' } }] }])
    );
  ...
  it('✅ should handle deepseek-reasoner message conversion', async () => {
    // Prepara
    const handler = createTestHandler('deepseek-reasoner');
    handler.setMockStream(createMockStream([{ choices: [{ delta: { content: 'Risposta' } }] }]));
