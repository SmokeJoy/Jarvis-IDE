			...
			try {
				...
				for await (const chunk of stream) {
					if (chunk && typeof chunk === 'object' && 'type' in chunk) {
						switch (chunk.type) {
							case "message_start":
								if (chunk.message && typeof chunk.message === 'object' && 'usage' in chunk.message) {
							...
							case "content_block_start":
								if (chunk.content_block && typeof chunk.content_block === 'object' && 'type' in chunk.content_block) {
				...
				if (chunk && typeof chunk === 'object' && 'chunk' in chunk && chunk.chunk?.bytes) {
					try {
						...
						if (parsedChunk && typeof parsedChunk === 'object') {
							...
							} else if ('delta' in parsedChunk && typeof parsedChunk.delta === 'object' && parsedChunk.delta && 'text' in parsedChunk.delta) {
...
`
		...
		try {
			...
			if (response && response.stream) {
				...
				for await (const chunk of response.stream) {
					...
					if (chunk && typeof chunk === 'object' && 'metadata' in chunk && chunk.metadata && 'usage' in chunk.metadata) {
					...
					if (chunk && typeof chunk === 'object' && 'contentBlockDelta' in chunk && 
						chunk.contentBlockDelta && typeof chunk.contentBlockDelta === 'object' && 
						'delta' in chunk.contentBlockDelta && chunk.contentBlockDelta.delta) {
						...
						if (typeof delta === 'object' && 'reasoningContent' in delta && 
							delta.reasoningContent && typeof delta.reasoningContent === 'object' && 
							'text' in delta.reasoningContent && delta.reasoningContent.text) {
							yield {
								type: "reasoning",
	 ...
	 */
	private formatNovaMessages(messages: ChatMessage[]): { role: ConversationRole; content: any[] }[] {
					...
					for (const line of lines) {
						try {
							...
							if (parsed && typeof parsed === 'object' && 
								'type' in parsed && parsed.type === "content_block_delta" && 
								'delta' in parsed && parsed.delta && 
								'text' in parsed.delta && parsed.delta.text) {
