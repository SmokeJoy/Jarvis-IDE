...
export class VsCodeLmHandler implements ApiHandler, SingleCompletionHandler {
  ...
  private async countTokens(text: string | VsCodeLanguageModelChatMessage): Promise<number> {
    ...
    if (!this.currentRequestCancellation) {
      console.warn(
        'Jarvis IDE <Language Model API>: No cancellation token available for token counting'
      );
      return 0;
    }
    ...
    try {
      ...
      if (typeof tokenCount !== 'number') {
        console.warn(
          'Jarvis IDE <Language Model API>: Non-numeric token count received:',
          tokenCount
        );
        return 0;
      }
  ...
  getModel(): { id: string; info: ModelInfo } {
    ...
    console.debug(
      'Jarvis IDE <Language Model API>: No client available, using fallback model info'
    );
