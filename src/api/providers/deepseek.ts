...
import type { getDeepseekConfig } from "./config/deepseek-config.js.js"
import type { deepseekModelInfoSaneDefaults } from "../../shared/api.js.js"
import { calculateApiCostDeepSeek } from "../../utils/cost.js.js"
...
export class DeepseekHandler extends BaseStreamHandler<ChatCompletionChunk> implements ApiHandler {
	...
	constructor(options: ApiHandlerOptions) {
		...
		logger.debug(`[DeepseekHandler] Inizializzato con baseURL: ${this.options.openAiBaseUrl || 'default'}`)
	}
	...
	protected async fetchAPIResponse(systemPrompt: string, messages: Anthropic.Messages.MessageParam[]): Promise<Stream<ChatCompletionChunk>> {
		...
		logger.info(`[DeepseekHandler] Inizio chiamata API DeepSeek con ${openAiMessages.length} messaggi`)
		...
		try {
			...
			logger.error(`[DeepseekHandler] Errore durante la chiamata API DeepSeek: ${error instanceof Error ? error.message : String(error)}`)
			throw error
		}
	}
