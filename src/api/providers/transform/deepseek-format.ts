...
import { DeepSeekConfig } from "../config/deepseek-config.js"
...
export class DeepSeekTransformer {
	...
	static fromLLMResponse(response: any): ChatMessage {
		try {
			...
			logger.error(`[DeepSeekTransformer] Errore durante la conversione della risposta: ${error.message}`);
			return {
				role: 'assistant',
				content: [{ type: ContentType.Text, text: "" } as TextBlock],
				timestamp: new Date().toISOString()
			};
		}
	}
