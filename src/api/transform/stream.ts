...
export type { ApiStream, ApiStreamChunk, ApiStreamTextChunk, ApiStreamReasoningChunk, ApiStreamUsageChunk };
 ...
 */
export function createStreamParser<T = any>(
					...
					while (!cancelled) {
						try {
							...
							for (const line of lines) {
								...
								if (raw === "[DONE]") return { done: true, value: undefined as any };
 ...
 */
export function createMockStream<T = ApiStreamChunk>(
	chunks: T[],
	delayMs = 0
): ApiStream<T> {
	let cancelled = false;
