import type { ChatMessage, ContentType, TextContent, ImageContent } from '../../types/chat.types.js.js';
...
export const OllamaTransformer = {
	...
	fromLLMResponse(response: any, model?: string): ChatMessage {
		try {
			...
			logger.error('[OllamaTransformer] Errore nella conversione della risposta', { err, response });
			throw err;
		}
	}
};
...
export function processOllamaImages(messages: ChatMessage[]): OllamaMessage[] {
	return messages.map(message => {
		...
		const textContent = message.content.replace(/!\[.*?\]\((.*?)\)/g, (match, url) => {
			imageUrls.push(url);
			return '';
		}).trim();
