...
describe("Extension Tests", function () {
	...
	it("should handle basic commands", async () => {
		...
		await vscode.commands.executeCommand("jarvis-ide.historyButtonClicked")
		// Success if no error thrown
	})

	it("should handle advanced settings configuration", async () => {
		// Test browser session setting
		await vscode.workspace.getConfiguration().update("jarvis-ide.disableBrowserTool", true, true)
		const updatedConfig = vscode.workspace.getConfiguration("jarvis-ide")
		expect(updatedConfig.get("disableBrowserTool")).to.be.true

		// Reset settings
		await vscode.workspace.getConfiguration().update("jarvis-ide.disableBrowserTool", undefined, true)
	})
})
