...
import { MasStateManager } from './MasStateManager';
...
export class MasManager extends EventEmitter {
  ...
  get queueService(): TaskQueueService {
    return this._queueService;
  }

  retryTask(taskId: string): boolean {
    const task = this._queueService.state.tasks.find((task) => task.id === taskId);
    if (!task) {
      return false;
    }

    // Reset task status to pending
    this._queueService.updateTaskStatus(taskId, 'pending');
    return true;
  }
}
