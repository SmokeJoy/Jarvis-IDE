...
import { Task, TaskStatus } from '../../shared/types/mas.types';
import { TaskQueueViewMessage } from '../../shared/types/webview.types';
import { MasPersistenceService } from './MasPersistenceService';
...
export class TaskQueueService {
  ...
  private loadPersistedState(): void {
    ...
    if (state) {
      try {
        this.logger.info(
          `Caricamento di ${state.tasks.length} task e ${state.agents.length} agenti dal disco`
        );
