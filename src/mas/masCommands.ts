...
import { MasPersistenceService } from './services/MasPersistenceService';
      ...
      try {
        ...
        if (aborted) {
          vscode.window.showInformationMessage(
            `Task ${taskSelected.detail} annullato con successo`
          );
        } else {
    ...
    vscode.commands.registerCommand('jarvis-ide.mas.toggleAgent', async () => {
      try {
        ...
        if (agent) {
          ...
          vscode.window.showInformationMessage(
            `Agente ${agent.name} ${newState ? 'attivato' : 'disattivato'} con successo`
          );
        }
      } catch (error) {
        vscode.window.showErrorMessage(
          `Errore nell'attivazione/disattivazione dell'agente: ${error}`
        );
      }
    })
  );
    ...
    vscode.commands.registerCommand('jarvis-ide.mas.importState', async () => {
      try {
        ...
        if (state) {
          ...
          vscode.window.showInformationMessage(
            `Stato MAS importato: ${state.tasks.length} task e ${state.agents.length} agenti`
          );
