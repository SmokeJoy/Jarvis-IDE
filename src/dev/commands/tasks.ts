 ...
 */
export function registerTaskCommands(context: vscode.ExtensionContext, provider: JarvisProvider): vscode.Disposable[] {
	return [
		vscode.commands.registerCommand("jarvis-ide.dev.createTestTasks", async () => {
 ...
 */
function createRealisticMessageSequence(baseTimestamp: number, taskPrompt: string, taskIndex: number): JarvisMessage[] {
	...
	const messages: JarvisMessage[] = [
		// Initial task message - uses "say" with "text" which is the format used in JarvisIde.ts
		{
			ts: baseTimestamp,
			type: "say",
			say: "text",
